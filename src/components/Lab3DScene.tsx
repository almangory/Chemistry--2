  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* 🎯 Interactive Mission Step Guidance Banner (Positioned safely below the top control bar without blocking Fullscreen) */}
        {currentGuidance && (
          <div className="absolute top-13 sm:top-14 left-1/2 -translate-x-1/2 z-20 w-[92%] max-w-md sm:max-w-lg pointer-events-auto">
            {isMissionCollapsed ? (
              <button
                type="button"
                onClick={() => setIsMissionCollapsed(false)}
                className="mx-auto bg-slate-900/90 hover:bg-slate-800 backdrop-blur-md px-3 py-1 rounded-full border border-amber-500/60 shadow-lg flex items-center justify-center gap-1.5 text-xs text-amber-300 font-bold transition-all cursor-pointer hover:scale-105"
                title="عرض المهمة المطلوبة للخطوة"
              >
                ...
                <span className="truncate max-w-[220px] sm:max-w-[320px]">المهمة: {currentGuidance.hintText}</span>
                <ChevronDown className="w-3 h-3 text-amber-400" />
              </button>
            ) : (
              <div className="bg-slate-900/95 backdrop-blur-md px-3 sm:px-3.5 py-2 rounded-xl border border-amber-500/70 shadow-2xl flex items-center justify-between gap-2 transition-all">
                <div className="flex items-center gap-2 text-right flex-1 min-w-0">
                  <span className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-amber-500 text-slate-950 font-bold text-xs sm:text-sm flex items-center justify-center shrink-0 shadow-md animate-bounce">
                    {currentGuidance.badgeSymbol}
                  </span>
                  <div className="flex flex-col min-w-0">
                    <div className="flex items-center gap-1.5 flex-wrap">
                      ...
                      <span className="text-[9px] bg-emerald-500/20 text-emerald-300 px-1.5 py-0.2 rounded font-bold border border-emerald-500/40 animate-pulse hidden sm:inline">
                        اسحب الأداة أو انقر عليها ⚡
                      </span>
                    </div>
                    <p className="text-[11px] sm:text-xs text-white font-bold font-sans truncate leading-tight mt-0.5">
                      {dragFeedback ? `🎯 ${dragFeedback}` : currentGuidance.hintText}
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-1 shrink-0">
                  {onActionTrigger && (
                    <button
                      ...
                      className="px-2.5 py-1 bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 text-slate-950 rounded-lg text-xs font-bold flex items-center gap-1 transition-all cursor-pointer shadow-md hover:scale-105 active:scale-95"
                  ...
                  <button
                    type="button"
                    onClick={() => setIsMissionCollapsed(true)}
                    title="تصغير شريط المهمة"
